...
//    self.titleScrollView.backgroundColor = [UIColor cyanColor];
...
-(void)setupTitle{
    ...
    CGFloat w = 110;
    CGFloat h = titleH;
    self.imageBackView  = [[UIImageView alloc] initWithFrame:CGRectMake(5, 5, w-10, titleH-10)];
...
-(void)setupTitleCenter:(UIButton *)sender
{
    ...
    CGFloat maxOffset  = fabs(self.titleScrollView.contentSize.width - ScreenW);
    if (offset > maxOffset && maxOffset>0) {
        offset = maxOffset;
    }
    
//    DLog("%lf,%lf",self.titleScrollView.contentSize.width,ScreenW);
